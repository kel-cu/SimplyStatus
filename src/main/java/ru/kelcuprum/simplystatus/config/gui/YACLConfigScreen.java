...
public class YACLConfigScreen {
    public static Screen buildScreen (Screen currentScreen) {
        ...
        if(SimplyStatus.replayMod) screen.category(new ReplayConfigs().getCategory());
        if(SimplyStatus.isMusicModsEnable) screen.category(new MusicConfigs().getCategory());
        if(SimplyStatus.isVoiceModsEnable) screen.category(new VoiceConfigs().getCategory());
