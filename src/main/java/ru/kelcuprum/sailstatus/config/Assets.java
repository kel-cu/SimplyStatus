...
public class Assets {
    ...
    public static void registerAsset(Assets assets){
        if(assets.file == null) {
            ...
            } else SailStatus.log(String.format("[File] Registration of %s by id %s failed, conflict of internal assets", assets.name, assets.id));
        }
    }
    ...
    public static String[] getAssetsNames(){
        ...
        for(String id : SailStatus.assetsNames){
            list[i] = SailStatus.assets.getOrDefault(id, ModConfig.defaultAssets).name;
            i++;
        }
        return list;
    }
    public static String[] getAssetsNames(String name){
        ...
        int i = 0;
        for(String id : SailStatus.assetsNames){
            if(!SailStatus.assets.getOrDefault(id, ModConfig.defaultAssets).name.equals(name)) {
                list[i] = SailStatus.assets.getOrDefault(id, ModConfig.defaultAssets).name;
                i++;
            }
        }
